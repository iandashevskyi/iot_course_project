...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data) {
    switch ((esp_mqtt_event_id_t)event_id) {
        case MQTT_EVENT_CONNECTED:
            ...
            // Уличной плате не нужно слушать топик действий (реле находятся внутри)
            break;
    ...
    while(1) {
        if (mqtt_client != NULL) {
            if (bmp280_read_float(&dev, &temp, &press, &hum) == ESP_OK) {
                ...
                cJSON_AddNumberToObject(root, "out_temp", temp);
                cJSON_AddNumberToObject(root, "out_hum", hum);
                cJSON_AddNumberToObject(root, "out_co2", 400.0); // заглушка для улицы
